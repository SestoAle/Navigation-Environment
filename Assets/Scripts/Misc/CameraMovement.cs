...
public class CameraMovement : MonoBehaviour
{
    ...
    public GameObject _cameraPonter;
    ...
    void Start()
    {
        _offset = _cameraPonter.transform.position - transform.position;
    ...
    private void FixedUpdate()
    {
        ...
        Vector3 newPos = _cameraPonter.transform.position - _offset;
        transform.position = Vector3.Slerp(transform.position, newPos, 0.5f);
        
        // Look at the player
        if (_lookAgent)
            transform.LookAt(_cameraPonter.transform);
