...
#pragma warning restore
        ...
        public string VectorObservationPlacholderName = "agent.independent_act/action-output";
        ...
        public string ActionPlaceholderName = "agent.independent_act/action-output";
...
//#endif
        ...
        public void DecideAction(Dictionary<Agent, AgentInfo> agentInfo)
        {
            ...
            if (hasState)
            {
                ...
                //// Get global state
                //int globalStateSize = 7;
                //float[,] global_state = new float[1,globalStateSize];
                //for(int i = 0; i < globalStateSize; i ++)
                //{
                //    global_state[0, i] = inputState[0, i];
                //}

                //// Get rays
                //int raysSize = 25;
                //float[,] rays = new float[1, raysSize];
                //for (int i = 0; i < raysSize; i++)
                //{
                //    rays[0, i] = inputState[0, globalStateSize + i];
                //}

                //// Get coins
                //int coinsSize = 28;
                //float[,] coins = new float[1, coinsSize];
                //for (int i = 0; i < coinsSize; i++)
                //{
                //    coins[0, i] = inputState[0, globalStateSize + raysSize + i];
                //}

                //// Get obstacles
                //int obstacleSize = 21;
                //float[,] obs = new float[1, obstacleSize];
                //for (int i = 0; i < obstacleSize; i++)
                //{
                //    obs[0, i] = inputState[0, globalStateSize + raysSize + coinsSize + i];
                //}

                runner.AddInput(graph["ppo/state"][0], inputState);
            ...
            // Fetch action distribution probabilities
            //runner.Fetch(graph["agent.independent_act/action-distribution-probabilities-query"][0]);
            runner.Fetch(graph["ppo/actor/add"][0]);
            // Value Function
            // runner.Fetch(graph["ppo/critic/Squeeze"][0]);
            ...
            // Save Action Distribution Probabilities into Agent
            agentList[0].lastProbabilitiesDistribution = networkOutput[1].GetValue() as float[,];

            // Value function
            //var value = networkOutput[2].GetValue();
            //agentList[0].lastValue = (float)value;
